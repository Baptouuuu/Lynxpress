<?php
	...
	*/
	...
	/**
		...
		* <code>
		*	//after an upload
		*	$my_image = Media::load_upload('input_name');
		*	$my_image->save('image/directory/'.$my_image->_name);
		*
		*	//load a file
		*	$my_image = new Media();
		*	$my_image->_mime = 'image/png';
		*	$my_image->load('my/image.png');
		*
		*	//manipulate an image
		*	$my_image->thumb(150, 0); //create a thumb with a width of 150px, 0 means keep original ratio
		*	$my_image->rotate(90);
		*	$my_image->flip('h');
		*	$my_image->flip('v');
		*
		*	//delete a media
		*	Media::delete('path/to/my/media.mp4');
		* </code>
		*
		* @package		Library
		* @subpackage	Media
		...
		public function thumb($width, $height){
			...
			if(isset($src) && $src !== false){
				...
				switch($this->_mime){
					...
					case 'image/jpeg':
						$result = @imagejpeg($dest, $dirname.$name, 100);
						break;
		...
		public function rotate($degree){
			...
			if(isset($src) && $src !== false){
				...
				switch($this->_mime){
					...
					case 'image/jpeg':
						$result = @imagejpeg($src, $this->_file, 100);
						break;
		...
		public function flip($align){
			...
			if(isset($src) && $src !== false){
				...
				switch($this->_mime){
					...
					case 'image/jpeg':
						$result = @imagejpeg($dest, $this->_file, 100);
						break;
